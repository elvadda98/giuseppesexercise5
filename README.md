...
<head>
    ...
    <title>Office Communication Lab</title>
    <style>
        :root {
        ...
        .gap-input { border: none; border-bottom: 2px solid var(--secondary-color); width: 140px; text-align: center; font-size: inherit; outline: none; background: transparent; color: var(--accent-color); font-weight: bold; }
...
<header>
    <h1>Office Communication Lab</h1>
...
<div class="container">
    ...
    <div id="tab-1" class="exercise-section" style="display:none">
        ...
        <button class="action-btn" onclick="checkWriting()">Verify Spelling</button>
    </div>
    ...
    <div id="tab-3" class="exercise-section" style="display:none">
        <h2>🗣️ Guess & Pronounce (No Labels)</h2>
...
<script>
    const data = [
        { word: "usually", def: "In the way that most often happens.", sents: ["I ___ drink coffee in the morning.", "Meetings are ___ held in Room B."] },
        { word: "colleague", def: "A person you work with in a profession.", sents: ["A ___ of mine is leading the project.", "I need to talk to my ___ about the report."] },
        { word: "deal with", def: "To take action to solve a problem.", sents: ["How should we ___ this customer complaint?", "I have to ___ a lot of emails today."] },
        { word: "got it", def: "A phrase used to say you understand.", sents: ["The deadline is Friday? ___, thanks!", "Click the save button twice. ___."] }
    ];
    ...
    function init() {
        ...
        shuffle(pPool);
        document.getElementById('pron-content').innerHTML = pPool.map((s, i) => `<div class="gap-sentence">${s.t.replace(s.a, '_________')} <button class="mic-btn" onclick="testSpeech('${s.a}', 'p-fb-${i}')">🎤</button><div id="p-fb-${i}" style="font-size: 0.85em; color:#888; margin-top:5px;">Guess from the bank and speak.</div></div>`).join('');
    }
